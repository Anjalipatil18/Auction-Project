...
auction.post('/',( req, res ) => {
    let auctionDetail = {
     ...
     knexAdd.insertData(auctionDetail)
     .then((data)=>{
...
auction.post('/bidder',(req , res) => {
        ...
        .then((data)=>{
            ...
            knex('Auction').select('price').where('auction_id',id)
            .then((price)=>{
                if(price[0]['price']<req.body.bid_value){
                     // console.log(data[0]['auction_id'])
                    knex('bidder').insert({ "auction_id":id,"bid_value":req.body.bid_value })          
                    .then((send_data)=>{
                        res.send(send_data)
                     }).catch((err)=>{
                        res.send(err)
...
auction.get('/sold_out',(req,res)=>{
        ...
        knexAdd.auctionName(auction_id)
            .then((name)=>{
                var auction_name=name[0]['auction_name']
                knexAdd.nameInsert(auction_id,bidder_id, max, auction_name )
                .then((data)=>{
